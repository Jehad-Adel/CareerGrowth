...
- **A form containing a `display: contents` fieldset must space itself with `flex flex-col gap-*`, not `space-y-*`.** Tailwind v4's rule is `.space-y-4 > :not(:last-child) { margin-block-end }`. `contents` removes the fieldset from the box tree, so that margin lands on an element generating no box and is discarded, *and* the fields inside are grandchildren the selector never matches. Every gap in the form collapses: labels sit flush against the hint above and the submit button touches the last field. A flex container hoists the fieldset's children into its own layout, so `gap` reaches the fields and the button alike. (Wrapping the fields in an inner `space-y-*` div — what `jobs/job-input.tsx` does — fixes the fields but *not* the gap to the button, because the fieldset's own margin is still discarded.) Applies to `PendingFieldset` and to any hand-written `<fieldset className="contents">`.
