...
- **A service's `FEATURE` must exist in `quota_service.DAILY_LIMITS`.** `consume()` raises a bare `ValueError` on an unknown key, and `ValueError` is not an `AppError`, so the miss surfaces as a blanket 500 that names nothing. Quiz, video, and offer evaluation all shipped with keys that were never added, so all three features 500'd on every request while the suite stayed green. `tests/test_quota_service.py::test_every_service_feature_has_a_limit` parses the service modules and fails the build on a new one.
- **Read uploads with a cap (`await file.read(MAX + 1)`), never `read()` then `len()`.** The second form buffers the whole body before the limit can reject it, so one oversized request exhausts the process. The CV path always did this; the audio path did not.
- **`app/ai/llm/transcription.py` goes through `langchain_google_genai`, not `google.generativeai`.** The latter is not in `pyproject.toml` and not installed; importing it raised `ModuleNotFoundError`, which `interview_service` caught and reported as "could not transcribe your audio" — so voice answers looked flaky rather than absent. `tests/test_transcription.py` asserts the declared dependency is the one in use.
- **`TRUSTED_PROXY_COUNT` must be `1` on Railway.** At the default of `0` the `X-Forwarded-For` header is ignored and every request keys to the platform edge's address, so the whole user base shares one 120/minute bucket. Nothing fails — the limiter just applies to the wrong key — so `install_rate_limiting` logs `rate_limit_key_is_shared` at startup when it is 0 in production.
...
- **The build context is `backend/`, and it comes from the service's Root Directory setting — not from anything in the repo.** `railway.json` still says `dockerfilePath: backend/Dockerfile` because that path *is* resolved from the repo root; only the context moves. The two disagreeing is the single most expensive failure here, and it reads as a missing file rather than a config problem: with the context at the repo root, `COPY alembic.ini` fails as `"/alembic.ini": not found`; with it at `backend/`, `COPY backend/alembic.ini` fails as `"/backend/alembic.ini": not found`. Same file, opposite directions. Reproduce a deploy exactly with `cd backend && docker build .`.
- **`.dockerignore` must sit next to the *context*, so it belongs in `backend/`.** One at the repo root is silently inert. Because nothing outside `backend/` is in the context, the repo-root `.env` never reaches the builder.
- **`railway.json` sets no `startCommand` on purpose — the image's `CMD` is the only start path.** That field overrides `CMD` and is parsed as argv with no shell and no variable expansion, which rules out both halves of what the command needs: a literal `$PORT` reaches uvicorn (`'$PORT' is not a valid integer`), and a leading `exec` is taken as the executable (``The executable `exec` could not be found``). The `CMD` is `sh -c` with an explicit `exec` so `${PORT:-8000}` expands *and* uvicorn ends up PID 1 for SIGTERM. Adding a `startCommand` back breaks one or the other.
